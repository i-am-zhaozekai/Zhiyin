...
public class SongSingerDB extends SQLiteOpenHelper {
    ...
    private volatile static SongSingerDB sInstance;

    private SongSingerDB(Context context) {
        super(context, "zy_songsinger.db", null, 2);
    }

    public static SongSingerDB getInstance(Context context) {
        if (sInstance == null) {
            synchronized (SongSingerDB.class) {
                if (sInstance == null) {
                    sInstance = new SongSingerDB(context);
                }
            }
        }
