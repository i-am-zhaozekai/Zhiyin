...
public class MultiplyLineLyricView extends View {
     ...
     * 歌词颜色
     */
    private int mLrcColor = Color.parseColor("#FFFADA83");
    private int mDefLrcColor = Color.parseColor("#FFFFFFFF");
    /**
     * 歌词的最大宽度
     */
     ...
     * 歌词快进事件
     */
    private MultiplyLineLyricView.OnLrcClickListener mOnLrcClickListener;
    /**
     * 额外歌词监听事件
     */
    private MultiplyLineLyricView.ExtraLyricsListener mExtraLyricsListener;
     ...
     * 额外歌词状态
     */
    private ExtraLyricShowMode mExtraLrcShowMode = ExtraLyricShowMode.NONE;
    /**
     * 分割歌词的行索引
     */
    ...
    public MultiplyLineLyricView(Context context) {
        this(context, null);
    }

    public MultiplyLineLyricView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MultiplyLineLyricView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, 0);
    }

    public MultiplyLineLyricView(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void drawIndicator(Canvas canvas) {
        ...
        String timeString = FileUtil.parseTime2TrackLength(startTime);
    ...
    private void drawDownLineLrc(Canvas canvas, float oldBottomY) {
        ...
        for (int i = mLyricsLineNum + 1; i < mLyricsLineTreeMap.size(); i++) {
            ...
            if (ExtraLyricShowMode.SHOW_TRANSLATION == mExtraLrcShowMode) {
                //画翻译歌词

                centreLastBottomY = drawDownLineTranslateLrc(canvas, i, centreLastBottomY);

            } else if (ExtraLyricShowMode.SHOW_TRANSLITERATION == mExtraLrcShowMode) {
                //画音译歌词
    ...
    private float getLineScollHeight(int lyricsLineNum) {
        ...
        for (int i = 0; i < lyricsLineNum; i++) {
            ...
            //判断是否有翻译歌词或者音译歌词
            if (mExtraLrcShowMode == ExtraLyricShowMode.SHOW_TRANSLATION) {
                if (mTranslateLrcLineInfos != null && mTranslateLrcLineInfos.size() > 0) {
            ...
            } else if (mExtraLrcShowMode == ExtraLyricShowMode.SHOW_TRANSLITERATION) {
                if (mTransliterationLrcLineInfos != null && mTransliterationLrcLineInfos.size() > 0) {
    ...
    private void extraLrcTypeCallBack(int curPlayingTime) {
        if (mLyricsUtil != null && mExtraLyricsListener != null && mLyricsLineTreeMap != null) {
            ...
            if (extraLrcType == LyricsUtil.TRANSLATE_AND_TRANSLITERATION_LRC) {
                ...
                mExtraLrcShowMode = ExtraLyricShowMode.SHOW_TRANSLATION;
                //有翻译歌词和音译歌词
                if (mExtraLyricsListener != null) {
                    mExtraLyricsListener.hasTranslateAndTransliterationLrcCallback(mExtraLrcShowMode);
                }
                ...
                //有翻译歌词
                if (mExtraLyricsListener != null) {
                    mExtraLyricsListener.hasTranslateLrcCallback(mExtraLrcShowMode);
                }

            } else if (extraLrcType == LyricsUtil.TRANSLITERATION_LRC) {

                mExtraLrcShowMode = ExtraLyricShowMode.SHOW_TRANSLITERATION;

                //音译歌词
                if (mExtraLyricsListener != null) {
                    mExtraLyricsListener.hasTransliterationLrcCallback(mExtraLrcShowMode);
                }

            } else {

                mExtraLrcShowMode = ExtraLyricShowMode.NONE;
                //无翻译歌词和音译歌词
        ...
        if (isManyLineLrc)
            setExtraLrcStatus(mExtraLrcShowMode, curPlayingTime);
    }
...
//        return (int) (-fm.leading - fm.ascent);
    ...
    public void setOnLrcClickListener(MultiplyLineLyricView.OnLrcClickListener mOnLrcClickListener) {
        this.mOnLrcClickListener = mOnLrcClickListener;
    }

    public void setExtraLyricsListener(MultiplyLineLyricView.ExtraLyricsListener mExtraLyricsListener) {
    ...
    public void updateView(int playProgress) {
        ...
        if (!isManyLineLrc) {
            ...
            //判断是否显示音译歌词
            if (mExtraLrcShowMode == ExtraLyricShowMode.SHOW_TRANSLITERATION) {
                if (mTransliterationLrcLineInfos != null && mTransliterationLrcLineInfos.size() > 0) {
     ...
     * @param curPlayingTime
     */
    public void setExtraLrcStatus(ExtraLyricShowMode mExtraLrcStatus, int curPlayingTime) {
        this.mExtraLrcShowMode = mExtraLrcStatus;
        if (mLyricsUtil == null || mLyricsLineTreeMap == null) {
            return;
        }
        if (mExtraLrcStatus != ExtraLyricShowMode.NONE && !isManyLineLrc) {
    ...
    public ExtraLyricShowMode getExtraLrcShowMode() {
        return mExtraLrcShowMode;
    }
    ...
    public interface ExtraLyricsListener {
         ...
         * 有翻译歌词回调
         */
        void hasTranslateLrcCallback(ExtraLyricShowMode showType);

        /**
         * 有音译歌词回调
         */
        void hasTransliterationLrcCallback(ExtraLyricShowMode showType);

        /**
         * 有翻译歌词和音译歌词回调
         */
        void hasTranslateAndTransliterationLrcCallback(ExtraLyricShowMode showType);
