...
public class MultiplyLineLyricView2 extends View {

    public static final int SHOW_TRANSLATE_LRC = 0;
    public static final int SHOW_TRANSLITERATION_LRC = 1;
    public static final int NO_SHOW_EXTRA_LRC = 2;
     ...
     * 歌词快进事件
     */
    private MultiplyLineLyricView2.OnLrcClickListener mOnLrcClickListener;
    /**
     * 额外歌词监听事件
     */
    private MultiplyLineLyricView2.ExtraLyricsListener mExtraLyricsListener;
     ...
     * 额外歌词状态
     */
    private int mExtraLrcStatus = NO_SHOW_EXTRA_LRC;
    /**
     * 分割歌词的行索引
     */
    ...
    public MultiplyLineLyricView2(Context context) {
        this(context, null);
    }

    public MultiplyLineLyricView2(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MultiplyLineLyricView2(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, 0);
    }

    public MultiplyLineLyricView2(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void drawDownLineLrc(Canvas canvas, float oldBottomY) {
        ...
        for (int i = mLyricsLineNum + 1; i < mLyricsLineTreeMap.size(); i++) {
            ...
            if (mExtraLrcStatus == SHOW_TRANSLATE_LRC) {
                //画翻译歌词

                centreLastBottomY = drawDownLineTranslateLrc(canvas, i, centreLastBottomY);

            } else if (mExtraLrcStatus == SHOW_TRANSLITERATION_LRC) {
                //画音译歌词
    ...
    private void extraLrcTypeCallBack(int curPlayingTime) {
        if (mLyricsUtil != null && mExtraLyricsListener != null && mLyricsLineTreeMap != null) {
            ...
            if (extraLrcType == LyricsUtil.TRANSLATE_AND_TRANSLITERATION_LRC) {
                ...
                mExtraLrcStatus = SHOW_TRANSLATE_LRC;
                ...
                mExtraLrcStatus = SHOW_TRANSLITERATION_LRC;
            } else {
                //无翻译歌词和音译歌词
                mExtraLyricsListener.noExtraLrcCallback();
                mExtraLrcStatus = NO_SHOW_EXTRA_LRC;
            }
...
//        return (int) (-fm.leading - fm.ascent);
    ...
    public void setOnLrcClickListener(MultiplyLineLyricView2.OnLrcClickListener mOnLrcClickListener) {
        this.mOnLrcClickListener = mOnLrcClickListener;
    }

    public void setExtraLyricsListener(MultiplyLineLyricView2.ExtraLyricsListener mExtraLyricsListener) {
    ...
    public void updateView(int playProgress) {
        ...
        if (!isManyLineLrc) {
            ...
            //判断是否显示音译歌词
            if (mExtraLrcStatus == SHOW_TRANSLITERATION_LRC) {
                if (mTransliterationLrcLineInfos != null && mTransliterationLrcLineInfos.size() > 0) {
    ...
    public void setExtraLrcStatus(int mExtraLrcStatus, int curPlayingTime) {
        ...
        if (mExtraLrcStatus != NO_SHOW_EXTRA_LRC && !isManyLineLrc) {
