...
public class LyricsUtil {
    ...
    public static File getLrcFile(Context context, String fileName) {
        List<String> lrcExts = LyricsIOUtils.getSupportLyricExts();
        for (int i = 0; i < lrcExts.size(); i++) {

            String lrcFilePath = FileUtil.getFilePath(context,
                    FileUtil.getAppPublicDirectory(context, null),
                    FileUtil.PATH_LYRICS + File.separator + fileName + "." + lrcExts.get(i));
    ...
    public void loadLrc(File lyricsFile) {
        ...
        LyricReader lyricsFileReader = LyricsIOUtils.getLyricsFileReader(lyricsFile.getName());
        try {
            mLyricsIfno = lyricsFileReader.readFile(lyricsFile);
            Map<String, Object> tags = mLyricsIfno.getLyricsTags();
            if (tags.containsKey(LyricTag.TAG_OFFSET)) {
                mDefOffset = 0;
                try {
                    mDefOffset = Integer.parseInt((String) tags.get(LyricTag.TAG_OFFSET));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
    ...
    public void loadLrc(byte[] base64ByteArray, File saveLrcFile, String fileName) {
        ...
        LyricReader lyricsFileReader = LyricsIOUtils.getLyricsFileReader(fileName);
        try {
    ...
    public LyricsInfo getLyricInfo() {
        return mLyricsIfno;
    }
