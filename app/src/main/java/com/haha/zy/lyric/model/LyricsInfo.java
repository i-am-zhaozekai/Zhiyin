package com.haha.zy.lyric.model;
...
public class LyricsInfo {
	...
	public void setTitle(String title) {
		...
		lyricsTags.put(LyricTag.TAG_TITLE, title);
	...
	public String getTitle() {
		...
		if (lyricsTags != null && !lyricsTags.isEmpty()
				&& lyricsTags.containsKey(LyricTag.TAG_TITLE)) {
			title = (String) lyricsTags.get(LyricTag.TAG_TITLE);
		}
		return title;
	...
	public void setArtist(String artist) {
		...
		lyricsTags.put(LyricTag.TAG_ARTIST, artist);
	}
	...
	public String getArtist() {
		...
		String artist = "";
		if (lyricsTags != null && !lyricsTags.isEmpty()
				&& lyricsTags.containsKey(LyricTag.TAG_ARTIST)) {
			artist = (String) lyricsTags.get(LyricTag.TAG_ARTIST);
		}
		return artist;
	...
	public void setOffset(long offset) {
		...
		lyricsTags.put(LyricTag.TAG_OFFSET, offset);
	}
	...
	public long getOffset() {
		...
		long offset = 0;
		if (lyricsTags != null && !lyricsTags.isEmpty()
				&& lyricsTags.containsKey(LyricTag.TAG_OFFSET)) {
			try {
				offset = Long.parseLong((String) lyricsTags
						.get(LyricTag.TAG_OFFSET));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public void setBy(String by) {
		...
		lyricsTags.put(LyricTag.TAG_BY, by);
	}
	...
	public String getBy() {
		...
		String by = "";
		if (lyricsTags != null && !lyricsTags.isEmpty()
				&& lyricsTags.containsKey(LyricTag.TAG_BY)) {
			by = (String) lyricsTags.get(LyricTag.TAG_BY);
		}
		return by;
	...
	public void setTotal(long total) {
		...
		lyricsTags.put(LyricTag.TAG_TOTAL, total);
	}
	...
	public long getTotal() {
		...
		long total = 0;
		if (lyricsTags != null && !lyricsTags.isEmpty()
				&& lyricsTags.containsKey(LyricTag.TAG_TOTAL)) {
			try {
				total = Long.parseLong((String) lyricsTags
						.get(LyricTag.TAG_TOTAL));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
